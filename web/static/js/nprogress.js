(function (b, a) {
    ...
    g.set = function (p) {
        ...
        p = c(p, k.minimum, 1);
        g.status = (p === 1 ? null : p);
        var q = g.render(!s), m = q.querySelector(k.barSelector), r = k.speed, o = k.easing;
        q.offsetWidth;
        h(function (n) {
            if (k.positionUsing === "") {
            ...
            e(m, b(p, r, o));
            if (p === 1) {
                e(q, {transition: "none", opacity: 1});
                q.offsetWidth;
                setTimeout(function () {
                    e(q, {transition: "all " + r + "ms linear", opacity: 0});
                    setTimeout(function () {
                        g.remove();
                        n()
                    }, r)
    ...
    g.render = function (n) {
        ...
        var m = q.querySelector(k.barSelector), p = n ? "-100" : l(g.status || 0), o = document.querySelector(k.parent),
            r;
        e(m, {transition: "all 0 linear", transform: "translate3d(" + p + "%,0,0)"});
        if (!k.showSpinner) {
    ...
    function b(p, q, o) {
        ...
        if (k.positionUsing === "translate3d") {
            m = {transform: "translate3d(" + l(p) + "%,0,0)"}
        } else {
            if (k.positionUsing === "translate") {
                m = {transform: "translate(" + l(p) + "%,0)"}
            } else {
                m = {"margin-left": l(p) + "%"}
            }
        }
